...
class Config:
    ...
    LR = 0.001023
    EPOCHS = 2000      # Ensemble için biraz kısabiliriz
    BATCH_SIZE = 32
    n_splits = 5       # 5 Farklı Model (K-Fold)
...
def main():
    ...
    print(f"🚀 ENSEMBLE EĞİTİM BAŞLIYOR ({Config.n_splits} Model)...")
    
    for fold, (train_idx, val_idx) in enumerate(kf.split(X)):
        print(f"\n--- FOLD {fold+1}/{Config.n_splits} ---")
        
        # Scaler her fold için ayrı fit edilmeli (Data leakage önlemek için)
        ...
        }, f"models/model_fold_{fold}.pt")
        print(f"✅ Model Kaydedildi: models/model_fold_{fold}.pt")
